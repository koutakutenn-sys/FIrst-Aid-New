...
public class StatusEffectLayer implements IGuiOverlay {
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        PlayerDamageModel damageModel = CommonUtils.getDamageModel(minecraft.player) instanceof PlayerDamageModel model ? model : null;
        if (damageModel == null) {
            return;
        }
        ...
        if (damageModel.getUnconsciousTicks() > 0) {
            ...
            Component title = Component.translatable(damageModel.getUnconsciousReasonKey());
            Component timer = damageModel.canGiveUp()
                    ...
                    : Component.translatable("firstaid.gui.unconscious_left", StringUtil.formatTickDuration(damageModel.getUnconsciousTicks()));
            ...
            if (damageModel.canGiveUp()) {
                int lineY = centerY + 2;
                guiGraphics.drawCenteredString(minecraft.font, Component.translatable("firstaid.gui.waiting_for_rescue"), centerX, lineY, opaque(0xE8D9D9));
                lineY += 12;
                ...
                guiGraphics.drawCenteredString(minecraft.font, Component.translatable("firstaid.gui.give_up_hint", ClientHooks.GIVE_UP.getTranslatedKeyMessage()), centerX, lineY, opaque(0xFFB3B3));
                renderGiveUpProgress(guiGraphics, minecraft, centerX, lineY + 14, partialTick);
            }
        } else if (ClientEventHandler.hasInteractionPrompt()) {
            renderRescuePrompt(guiGraphics, minecraft, screenWidth / 2, screenHeight / 2 + 24, partialTick);
        }
    }
    ...
    private static void renderDeathDangerOverlay(GuiGraphics guiGraphics, int width, int height, float deathDanger, float pulseTime) {
        ...
        guiGraphics.drawCenteredString(minecraft.font, ClientEventHandler.getInteractionPromptTitle(), centerX, top, opaque(0x55E0FF));
        guiGraphics.drawCenteredString(minecraft.font, ClientEventHandler.getInteractionPromptDetail(), centerX, top + 11, opaque(0x8FEFFF));
        int barTop = top + 24;
        int left = centerX - 72;
        int right = left + 144;
        int bottom = barTop + 8;
        ...
        guiGraphics.drawCenteredString(minecraft.font, ClientEventHandler.getInteractionPromptProgressText(partialTick), centerX, barTop + 12, opaque(0x9CF6FF));
        return barTop + 24;
    }

    private static int drawCenteredLines(GuiGraphics guiGraphics, Minecraft minecraft, Component[] lines, int centerX, int startY, int color, int lineHeight) {
        int y = startY;
   ...
   private static void renderGiveUpProgress(GuiGraphics guiGraphics, Minecraft minecraft, int centerX, int top, float partialTick) {
    ...
    private static void renderRescuePrompt(GuiGraphics guiGraphics, Minecraft minecraft, int centerX, int centerY, float partialTick) {
        ...
        guiGraphics.drawCenteredString(
                ...
                centerX,
                centerY - 26,
                healingPrompt ? opaque(0x74E5EC) : (executionPrompt ? opaque(0xFFD7CC) : opaque(0xE9F7E2))
        );
        guiGraphics.drawCenteredString(
                minecraft.font,
                ...
                centerX,
                centerY - 12,
                healingPrompt ? opaque(0x9E9FDE) : (executionPrompt ? opaque(0xEFD2D2) : opaque(0xCFE4C5))
        );
        ...
        guiGraphics.drawCenteredString(
                ...
                centerX,
                top + 12,
                healingPrompt ? opaque(0xAEE0CC) : (executionPrompt ? opaque(0xFFB7A0) : opaque(0xD8F0D0))
        );
    }
    ...
    private static float getDeathDangerProgress(PlayerDamageModel playerDamageModel) {
        if (!playerDamageModel.canGiveUp()) {
            return 0.0F;
        }
        int remaining = Mth.clamp(playerDamageModel.getUnconsciousTicks(), 0, CRITICAL_UNCONSCIOUS_TICKS);
        return Mth.clamp(1.0F - (remaining / (float) CRITICAL_UNCONSCIOUS_TICKS), 0.0F, 1.0F);
    }

    public void resetDebugState() {
        painStrength = 0.0F;
        lastPainStrength = 0.0F;
        suppressionStrength = 0.0F;
        lastSuppressionStrength = 0.0F;
    }
}
