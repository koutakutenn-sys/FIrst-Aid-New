...
public class EventHandler {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (source.is(DamageTypeTags.IS_PROJECTILE)) {
            Entity entityProjectile = source.getDirectEntity();
            ProjectileHitContext projectileHitContext = hitList.remove(player);
            if (projectileHitContext != null && projectileHitContext.projectile() == entityProjectile) {
            ...
            if (damageDistribution == null && entityProjectile != null) {
    ...
    public static void tickPlayers(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END && !event.player.getAbilities().invulnerable) {
            ...
            if (!event.player.level().isClientSide && damageModel instanceof PlayerDamageModel playerDamageModel) {
                float nearMissStrength = getNearbyProjectileStrength(event.player);
                if (nearMissStrength > 0.0F) {
                    playerDamageModel.registerAdrenalineNearMiss(event.player, nearMissStrength);
                }
    ...
    private static void awardStarterRecipes(ServerPlayer player) {
        List<Recipe<?>> recipes = Objects.requireNonNull(player.level().getServer())
                .getRecipeManager()
                .getRecipes()
                .stream()
                .filter(recipe -> FirstAid.MODID.equals(recipe.getId().getNamespace()))
                .toList();
        if (!recipes.isEmpty()) {
    ...
    public static void onWorldLoad(LevelEvent.Load event) {
        ...
        if (!world.isClientSide() && world instanceof Level)
            ((Level) world).getGameRules().getRule(GameRules.RULE_NATURAL_REGENERATION).set(FirstAid.naturalRegenMode != FirstAid.NaturalRegenMode.OFF, ((Level) world).getServer());
    }
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        if (!event.getLevel().isClientSide() && isUnconscious(event.getEntity())) {
            event.setCanceled(true);
        }
    }
    ...
    public static void adjustHitboxSize(EntityEvent.Size event) {
        // Hitbox size is handled through PlayerDimensionsMixin. Keeping the legacy
        // event-based override active at the same time causes repeated size refreshes.
    }
    ...
    public static void onServerStarted(ServerStartedEvent event) {
        ...
        event.getServer().getAllLevels().forEach(level ->
                level.getGameRules().getRule(GameRules.RULE_NATURAL_REGENERATION)
                        .set(FirstAid.naturalRegenMode != FirstAid.NaturalRegenMode.OFF, event.getServer()));
    }
    ...
    private static float getNearMissStrength(Player player, Projectile projectile, Vec3 start, Vec3 end, Vec3 target) {
        ...
        float speedFactor = net.minecraft.util.Mth.clamp((float) ((speed - 0.18D) / 0.65D), 0.0F, 1.0F);
        float distanceFactor = net.minecraft.util.Mth.clamp(1.32F - (float) (distance / 1.85D), 0.0F, 1.0F);
        return net.minecraft.util.Mth.clamp(distanceFactor * (0.82F + 0.58F * speedFactor), 0.0F, 1.45F);
    }
    ...
    private static ClosestPointResult closestPointOnSegment(Vec3 start, Vec3 end, Vec3 target) {
        ...
        double progress = net.minecraft.util.Mth.clamp(target.subtract(start).dot(segment) / lengthSqr, 0.0D, 1.0D);
        return new ClosestPointResult(start.add(segment.scale(progress)), progress);
    }

    private static <T extends PlayerInteractEvent> void cancelIfUnconscious(T event) {
        if (isUnconscious(event.getEntity())) {
            event.setCanceled(true);
        }
    }
    ...
    private static void completeRescue(ServerPlayer rescuer, InteractionTarget rescueTarget) {
        ...
        if (usingDefibrillator) {
            stack.hurtAndBreak(1, rescuer, living -> living.broadcastBreakEvent(getEquipmentSlot(rescueTarget.hand())));
        } else {
            stack.shrink(1);
        }
    ...
    private static void completeExecution(ServerPlayer executor, InteractionTarget executionTarget) {
        ...
        if (stack.isDamageableItem()) {
            stack.hurtAndBreak(1, executor, living -> living.broadcastBreakEvent(getEquipmentSlot(executionTarget.hand())));
        }
