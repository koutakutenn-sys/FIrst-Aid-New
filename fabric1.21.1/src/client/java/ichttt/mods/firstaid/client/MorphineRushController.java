...
public final class MorphineRushController {
    private static final List<ResourceLocation> RUSH_SOUND_IDS = List.of(
            ResourceLocation.fromNamespaceAndPath(FirstAid.MODID, "debuff.heartbeat"),
            ResourceLocation.fromNamespaceAndPath(FirstAid.MODID, "debuff.tinnitus"),
            ResourceLocation.withDefaultNamespace("block.beacon.activate"),
            ResourceLocation.withDefaultNamespace("block.beacon.ambient"),
            ResourceLocation.withDefaultNamespace("block.beacon.power_select"),
            ResourceLocation.withDefaultNamespace("block.respawn_anchor.charge"),
            ResourceLocation.withDefaultNamespace("entity.player.levelup"),
            ResourceLocation.withDefaultNamespace("block.enchantment_table.use"),
            ResourceLocation.withDefaultNamespace("block.amethyst_block.chime"),
            ResourceLocation.withDefaultNamespace("entity.experience_orb.pickup"),
            ResourceLocation.withDefaultNamespace("entity.allay.ambient_with_item")
    );
    ...
    private void playOnset(Minecraft client, RandomSource random) {
        // Stronger pair: body + warm wash
        playSound(client, ResourceLocation.fromNamespaceAndPath(FirstAid.MODID, "debuff.heartbeat"), 0.48F, 1.05F + random.nextFloat() * 0.15F);
        playSound(client, ResourceLocation.withDefaultNamespace("block.beacon.activate"), 0.42F, 0.85F + random.nextFloat() * 0.20F);
        if (random.nextBoolean()) {
            playSound(client, ResourceLocation.fromNamespaceAndPath(FirstAid.MODID, "debuff.tinnitus"), 0.22F, 0.90F + random.nextFloat() * 0.25F);
        }
    }
