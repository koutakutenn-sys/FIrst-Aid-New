...
public final class HealthRenderUtils {
    ...
    public static void blit(GuiGraphicsExtractor guiGraphics, Identifier texture, int textureWidth, int textureHeight, int x, int y, int u, int v, int width, int height) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, (float) u, (float) v, width, height, textureWidth, textureHeight);
    }

    public static void blit(GuiGraphicsExtractor guiGraphics, Identifier texture, int textureWidth, int textureHeight, int x, int y, int u, int v, int width, int height, int color) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, (float) u, (float) v, width, height, textureWidth, textureHeight, color);
    }

    public static void drawHealthString(GuiGraphicsExtractor guiGraphics, Font font, AbstractDamageablePart damageablePart, int xTranslation, int yTranslation, boolean allowSecondLine) {
        ...
        if (absorption > 0F) {
            ...
            if (allowSecondLine) {
                guiGraphics.text(font, line2, xTranslation, yTranslation + 5, ABSORPTION_TEXT_COLOR);
                yTranslation -= 5;
            } else {
                guiGraphics.text(font, text, xTranslation, yTranslation, healthColor);
                guiGraphics.text(font, " " + line2, xTranslation + font.width(text), yTranslation, ABSORPTION_TEXT_COLOR);
                return;
            }
        }
        guiGraphics.text(font, text, xTranslation, yTranslation, healthColor);
    ...
    public static void drawHealth(GuiGraphicsExtractor guiGraphics, Font font, AbstractDamageablePart damageablePart, int xTranslation, int yTranslation, boolean allowSecondLine) {
        ...
        if (minecraft.player != null && FirstAidConfig.SERVER.allowOtherHealingItems.get() && minecraft.player.hasEffect(MobEffects.REGENERATION)) {
            regen = (minecraft.gui.hud.getGuiTicks() / 2) % 15;
        }
    ...
    private static void renderMax(int regen, int[] lowOffsets, int startSlot, int max, int heartsPerRow, int baseX, int baseY, GuiGraphicsExtractor guiGraphics, Player player, boolean highlight) {
    ...
    private static void renderCurrentHealth(int regen, int[] lowOffsets, int current, int heartsPerRow, int baseX, int baseY, GuiGraphicsExtractor guiGraphics, Player player, boolean highlight) {
    ...
    private static void renderHeartSprites(int regen, int[] lowOffsets, int startSlot, int toDraw, boolean lastOneHalf, int heartsPerRow, int baseX, int baseY, GuiGraphicsExtractor guiGraphics, Identifier halfSprite, Identifier fullSprite) {
        if (toDraw <= 0) {
            return;
        }
