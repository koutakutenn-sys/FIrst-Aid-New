...
public class GuiHealthScreen extends Screen {
    ...
    public static final ItemStack BED_ITEMSTACK = new ItemStack(Items.BED.red());
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (minecraft != null && minecraft.player != null) {
            InventoryScreen.extractEntityInInventoryFollowsMouse(guiGraphics,
                    ...
                    30, 0.0625F, (float) mouseX, (float) mouseY, minecraft.player);
        }

        guiGraphics.centeredText(font, title, width / 2, guiTop + 6, 0xFFFFFF);
        ...
        if (morphineTicks > 0) {
            guiGraphics.centeredText(font, I18n.get("firstaid.gui.morphine_left", StringUtil.formatTickDuration(morphineTicks, 20F)), width / 2, guiTop + ySize - 22, 0xFFFFFF);
        } else if (activeHand != null) {
            guiGraphics.centeredText(font, I18n.get("firstaid.gui.apply_hint"), width / 2, guiTop + ySize - 22, 0xFFFFFF);
        }

        super.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
    }

    private void drawHealth(GuiGraphicsExtractor guiGraphics, AbstractDamageablePart damageablePart, boolean right, int yOffset) {
    ...
    private static void drawPartHealthIndicator(GuiGraphicsExtractor guiGraphics, int x, int y, AbstractDamageablePart damageablePart) {
