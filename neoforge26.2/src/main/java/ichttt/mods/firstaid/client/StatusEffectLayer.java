...
public class StatusEffectLayer implements GuiLayer {
    ...
    public void render(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
        ...
        // Pain pulse + morphine wash (26.2 compositor path).
        ...
        if (smoothSuppression > 0.0F) {
            float pulse = 0.92F + 0.08F * Mth.sin(pulseTime * 0.40F + 0.8F);
            float intensity = Math.min(0.55F, smoothSuppression * 0.40F * pulse);
            renderEdgeVignette(guiGraphics, width, height, 40, 46, 58, intensity * 0.50F, 16);
        }

        float tinnitusStrength = suppressionFeedbackController.getTinnitusStrength();
        if (tinnitusStrength > 0.0F) {
            float pulse = 0.88F + 0.12F * Mth.sin(pulseTime * 0.70F + 1.3F);
            float intensity = Math.min(0.28F, tinnitusStrength * 0.18F * pulse);
            renderEdgeVignette(guiGraphics, width, height, 190, 195, 205, intensity, 12);
        }
        ...
        if (damageModel.getUnconsciousTicks() > 0) {
            ...
            guiGraphics.centeredText(minecraft.font, title, centerX, centerY - 26, opaque(0xFFF1F1));
            guiGraphics.centeredText(minecraft.font, timer, centerX, centerY - 10, opaque(0xCFCFCF));
            if (playerDamageModel != null && playerDamageModel.canGiveUp()) {
                int lineY = centerY + 2;
                guiGraphics.centeredText(minecraft.font, Component.translatable("firstaid.gui.waiting_for_rescue"), centerX, lineY, opaque(0xE8D9D9));
                lineY += 12;
                lineY = drawCenteredLines(guiGraphics, minecraft, new Component[]{
                        Component.translatable("firstaid.gui.rescue_help.line1"),
                        Component.translatable("firstaid.gui.rescue_help.line2")
                }, centerX, lineY, opaque(0xD8CACA), 11);
                lineY += 4;
                ...
                guiGraphics.centeredText(minecraft.font, Component.translatable("firstaid.gui.give_up_hint", ClientHooks.GIVE_UP.getTranslatedKeyMessage()), centerX, lineY, opaque(0xFFB3B3));
    ...
    private static void renderVignette(GuiGraphicsExtractor guiGraphics, int width, int height, int red, int green, int blue, float intensity, int baseThickness) {
        renderEdgeVignette(guiGraphics, width, height, red, green, blue, intensity, baseThickness);
    }

    /** Edge-only vignette (no full-screen fill). */
    private static void renderEdgeVignette(GuiGraphicsExtractor guiGraphics, int width, int height, int red, int green, int blue, float intensity, int baseThickness) {
        if (intensity <= 0.0F) {
            return;
        }
        int layers = 5;
        for (int layer = 0; layer < layers; layer++) {
            ...
            int thickness = Math.max(3, Math.round(baseThickness * (0.30F + progress * (0.95F + intensity * 0.55F))));
            int alpha = Math.round((6.0F + 42.0F * intensity) * falloff * falloff);
            if (alpha > 0) {
    ...
    private static int renderSelfDefibProgress(GuiGraphicsExtractor guiGraphics, Minecraft minecraft, int centerX, int top, float partialTick) {
        guiGraphics.centeredText(minecraft.font, ClientEventHandler.getInteractionPromptTitle(), centerX, top, opaque(0x55E0FF));
        guiGraphics.centeredText(minecraft.font, ClientEventHandler.getInteractionPromptDetail(), centerX, top + 11, opaque(0x8FEFFF));
        int barTop = top + 24;
        int left = centerX - 72;
        int right = left + 144;
        int bottom = barTop + 8;
        ...
        guiGraphics.centeredText(minecraft.font, ClientEventHandler.getInteractionPromptProgressText(partialTick), centerX, barTop + 12, opaque(0x9CF6FF));
        return barTop + 24;
    }

    private static int drawCenteredLines(GuiGraphicsExtractor guiGraphics, Minecraft minecraft, Component[] lines, int centerX, int startY, int color, int lineHeight) {
        int y = startY;
    ...
    private static void renderRescuePrompt(GuiGraphicsExtractor guiGraphics, Minecraft minecraft, int centerX, int centerY, float partialTick) {
        ...
        guiGraphics.centeredText(
                ...
                centerX,
                centerY - 26,
                healingPrompt ? opaque(0x74D6DF) : (executionPrompt ? opaque(0xFFD7CC) : opaque(0xE9F7E2))
        );
        guiGraphics.centeredText(
                minecraft.font,
                ...
                centerX,
                centerY - 12,
                healingPrompt ? opaque(0x9E9E5E) : (executionPrompt ? opaque(0xEFD2D2) : opaque(0xCFE4C5))
        );
        ...
        guiGraphics.fill(left + 1, top + 1, right - 1, bottom - 1, healingPrompt ? color(180, 12, 54, 66) : (executionPrompt ? color(180, 82, 18, 18) : color(180, 24, 74, 28)));
        if (fillWidth > 0) {
            guiGraphics.fill(left + 1, top + 1, left + 1 + fillWidth, bottom - 1, healingPrompt ? color(220, 88, 224, 210) : (executionPrompt ? color(220, 232, 70, 70) : color(220, 126, 214, 110)));
        }
        ...
        guiGraphics.centeredText(
                ...
                centerX,
                top + 12,
                healingPrompt ? opaque(0xAEDECC) : (executionPrompt ? opaque(0xFFB7A0) : opaque(0xD8F0D0))
        );
    }
