...
public class HUDHandler implements IdentifiableResourceReloadListener, ResourceManagerReloadListener, HudElement {
   ...
   private static final int PLAYER_MODEL_SUMMARY_PADDING = 4;
   private static final int SUMMARY_LINE_HEIGHT = 10;
   ...
   public void extractRenderState(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
      if (FirstAidConfig.CLIENT.overlayMode.get() != OverlayMode.OFF) {
         ...
         if (minecraft.player != null && minecraft.player.isAlive() && minecraft.gameMode != null && !minecraft.gui.hud.isHidden()) {
            ...
            if (damageModel != null && FirstAid.isSynced) {
               ...
               if (visibleTicks == -1 || this.ticker >= 0) {
                  if (!(minecraft.gui.screen() instanceof ChatScreen) || FirstAidConfig.CLIENT.pos.get() != Position.BOTTOM_LEFT) {
        ...
        int summaryLineCount = showIndicators ? StatusSummaryRenderer.countVisibleLines(damageModel, minecraft.player) : 0;
                        ...
                        if (playerModel) {
                           ...
                           for (AbstractDamageablePart part : damageModel) {
                              guiGraphics.text(minecraft.font, this.translationMap.get(part.part), xOffset, y, 16777215);
