      ...
      do {
         ...
         for (AbstractDamageablePart part : damageModel) {
            if (part.currentHealth > 0.0F) {
               ...
               float scaledLeft = part.damage(scaledDamage, player, !player.hasEffect(RegistryObjects.PAINKILLER_EFFECT), tryNoKillThisRound ? 1.0F : 0.0F);
   ...
   public float distributeDamage(float damage, @Nonnull Player player, @Nonnull DamageSource source, boolean addStat) {
      ...
      if (damage <= 0.0F) {
         ...
         if (damageModel == null) {
            ...
            DistributionResult result = this.distributeOnParts(damage, damageModel, player, this.tryNoKill);
            if (result.damageLeft > 0.0F && this.tryNoKill) {
               result = this.distributeOnParts(damage, damageModel, player, false);
            }
